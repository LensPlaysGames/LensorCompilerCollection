...
instruction is easily the most abstract, as it is an instruction that is found in all SSA language, and yet, there are no physical CPUs
that implement anything remotely similar to them.
...
\verb|%a| before the branch to \verb|bb4|; the actual PHI instruction ultimately is not emitted at all, but we still allocate a register
for it and this is the register that we copy \verb|%c| or \verb|%d| into. After register allocation, we may end up with something like
this:
\begin{Verbatim}
bb1:
...
\verb|%c| directly in \verb|%r4|, which would make the copy unnecessary since the value would already be in the right register.
...
approach, one of which is rather easy to solve (and the solution is implemented in our RA), the other one... not so much.

\subsection{Critical Edges}
The first problem can occur when the argument of a phi node originates in a block that has a \textit{critical edge}. A critical edge is
...
\noindent Trying to lower this PHI instruction by simply inserting copies in the corresponding blocks doesn't quite work in this case:
\begin{Verbatim}
bb1:
    %r1 = ...
    %r2 = ...
...
\noindent At first glance, it might seem as though there were nothing wrong with this problem; and indeed, it works just fine, so long
as the \verb|(*)| branch instruction branches to \verb|bb4|. However, what happens if control flow branches to \verb|bb3| instead? In that
case, we still always copy \verb|%r3| to \verb|%r4|—even if its value isn't used in \verb|bb3|! This means that \verb|%r4| cannot be used
at all in \verb|bb3|! 
...
of \verb|%v| which was held in \verb|%r4| up to this point is overwritten with the value in \verb|%r3|, \textit{irrespective of what block
we branch to}. Everything is fine so long as we branch to \verb|bb4|, but if we branch to \verb|bb4|, then we have problem, since \verb|bb4|
...
is never clobbered if control flow branches to \verb|bb3| instead.
