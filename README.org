...
The driver, by default, uses file extension to determine which compiler to pass the source code to. ~lcc ./tst/intercept/byte.int~ will invoke the ~Intercept~ compiler, while ~lcc ./tst/laye/exit_code.laye~ will invoke the ~Laye~ compiler, for example. You can use ~-x LANG~ to treat source files as a particular language, regardless of extension.
