...
[[file:res/HISTORY.org][History of LCC]]
...
The driver, by default, uses file extension to determine which language to treat the source as. ~lcc foo.g~ will treat the contents as ~Glint~ source, while ~lcc foo.lcc~ will treat the contents as LCC IR, for example . You can use ~-x LANG~ to treat source files as a particular language, regardless of extension.

** Building

Dependencies ([[file:deps.org][see here]]):

- [[https://cmake.org/][CMake >= 3.20]]

- Any C++ Compiler (We like [[https://gcc.gnu.org/][GCC]], preferably a _recent_ version)

NOTE: If on Windows *and* using Visual Studio, see [[file:documentation/lcc/VISUAL_STUDIO.org][this document]] instead. Also, switch to Linux, it takes minutes :^).
...
In the future, we hope to support YOUR language =:)= (it's easy!! really)

*** Glint | Low Level, Higher Order

See [[file:documentation/glint/][relevant documentation]].
...
Support for C23 (N3220) began work in June of 2026.

It is still a WIP, with progress being made every day.
