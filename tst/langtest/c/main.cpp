...
struct CLanguageTest : langtest::Test {
    ...
    struct ParseInfo {
        lcc::language_c::TranslationUnit tu{};
        std::vector<lcc::Context::DiagnosticReport> diagnostics{};
    };
    /// Parse test source as C Source Code
    auto parse(lcc::Context& context) -> ParseInfo {
        // Parse C Source Code using the C parser into a translation unit.
    ...
    auto run() -> bool {
        ...
        if (do_matching) {
            ...
            ast_matches = perform_match<lcc::language_c::Node>(parse_info.tu.tree, matcher);
        }
        ...
        if (do_irgen) {
            // Perform IR matching
            auto* got_ir = lcc::language_c::IRGen::Generate(&context, parse_info.tu);
        ...
        if (not passed) {
            ...
            if (not ast_matches) {
                ...
                std::string got = langtest::print_node<lcc::language_c::Node>(
                    parse_info.tu.tree
                );
...
void emit_sarif_file(
    ...
    JSONObject tool{};
    JSONObject driver{};
    driver.add_property("name", "lcc.langtest.c");
...
void visit_directory(
    ...
    for (const auto& entry : std::filesystem::directory_iterator(directory_path)) {
        if (entry.is_regular_file()) {
            ...
            auto count = langtest::process_ast_test_file<CLanguageTest>(entry.path());
