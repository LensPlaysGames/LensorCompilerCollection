...
void emit_instruction(CodegenContext *context, IRInstruction *instruction) {
  ...
  case IR_RETURN:
    ...
    break;
  case IR_BRANCH:
    if (!optimise || ((instruction->block && instruction->value.block != ir_next_block(instruction->block)) && !instruction->block->done)) {
  ...
  case IR_BRANCH_CONDITIONAL:
    ...
    if (!optimise || (instruction->block && (instruction->value.conditional_branch.true_branch != ir_next_block(instruction->block) && !instruction->block->done))) {
...
void emit_function(CodegenContext *context, IRFunction *function) {
  ...
  VECTOR_FOREACH_PTR (IRBlock*, block, function->blocks) { emit_block(context, block); }
