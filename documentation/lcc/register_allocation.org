...
"Clobbers" is a silly term to mean "invalidates value".

- Clobber :: To invalidate a value in a register (usually via instruction execution).

We need to define this so that our register allocator is a bit smarter; with clobbers, it can understand =move= instructions without needing to know which opcodes for every architecture actually move things. This is done by setting clobbers on an instruction in the MIR.

A clobber can be a register constant (i.e. =%rax=) or an operand reference (i.e. =operand zero=).

When an instruction clobbers a register, it is known that the value in that register will be altered upon execution of this instruction.
