#+title: LCC TODO [DEPRECATED]
#+author: Lens_r

* [DEPRECATED] ~lcc~, Codegen
...
#+begin_src lisp
  (machine
   ...
   (section ".custom-section" list-of-byte-values)
   ...
   ;; div and friends should be forced to list result registers, so no
   ;; "hidden" clobbers other than flags/rip/rsp register. Repetition is
   ;; better than hidden semantics, imo.
