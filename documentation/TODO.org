...
- [CHECKS]: It's a mess, currently

  I want a _regular function_ I can call with a severity level enum which dictates *at compile time* whether or not the function call actually happens or not...

  Basically, allow the programmer to dictate how important the assertion/check they are performing is. If it's a confidence check, put it under that severity level. If it's a check that should be performed no matter what, even in release builds, put it under "Release" severity level. Etc.

  Replace LCC_ASSERT, LCC_TODO, etc.

- [MIRTest]: MIR -> MIR testing facility
  This will allow us to test register allocation, instruction selection, among other things.

  On Testing Register Allocation.
  To test register allocation, we will provide some input MIR, select a machine description, and expect some output by asserting registers are what they are supposed to. I suspect we will need "generic matchers" like "general purpose register of 32-bit width", rather than specific registers.

  On Testing Instruction Selection.
  To test instruction selection, we will provide some input gMIR (or possibly produce some gMIR from some input LCC IR), and expect exact and specific MIR instructions in the output. The idea would be to write at least one test per pattern per architecture...

- [Meta]: Improve =isel.cc= and =module_mir.cc= build times
  Reaching two minutes...

  =module_mir.cc= is just a monstrous function with lambdas that needs broken up, probably into a helper struct (=MIRBuildContext= or smth).

  =isel.cc= isn't bad since it's not changed a lot and it's goal is to be efficient...
...
- [Fractionals]: Test Coverage
  - float return value register allocation
    This isn't tested in =tst/codetest/x86_64/corpus/return_registers.txt= yet because codetest has no way of parsing fractionals: /sigh/.
- [Fractionals]: Uhhhh ... negative numbers exist
...
- [Necessary]: Export templated function
- [Necessary]: Export templated type
- [Necessary]: Use templated type as partial type specification to template/templated function parameter
...
- [Sema]: Expression expressions
  Dynamic array of =expr=. =for e in exprs, apply foo, e;= type of thing.
...
- [Sema]: A =builds_result= attribute, where =result= is then automagically declared as a variable of the return type.
...
- [Parser/Sema]: Module versioning
  ...
  I'm not /super/ sure about this one. I /think/ I do want versioning, but I may have it be a name convention rather than built-in? I don't know. I have to figure out how I want it to work.
...
- [Parser/Sema]: Compile-time Checked Interpolation: =printf "value: {}\n", 69;=
  ...
  Also a cool thing to support would be "quick debugging" formats.
  So =printf "{dbg:x}";= expands to =printf "x :: {}", x;=.
...
- [Parser/Sema]: Recursive Lambda
  What would the syntax be for a lambda to call itself, given it has no name?
