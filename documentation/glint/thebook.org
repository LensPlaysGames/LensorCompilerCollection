...
#+begin_src emacs-lisp :exports none :results none
  ...
  ;; Thanks, Amit!
  (defun lens/title-to-filename (title)
  ...
  (defun lens/generate-custom-ids ()
    "Generate CUSTOM_ID for any 'org-mode' headings that are missing one."
      ...
      (org-map-entries
       (lambda ()
         (let* ((custom-id      (org-entry-get nil "CUSTOM_ID"))
                ...
                (slug           (lens/title-to-filename headline))
  ...
  (defun lens/export-the-book ()
    ...
    (let
        ;; Use CSS classes to style output (vs. inline style tags).
        ((org-html-htmlize-output-type 'css)
         ;; Always generate new files.
         ...
         ;; Do not insert "validate" junk at end.
         (org-html-postamble nil))

      (copy-file "thebook.org" "thebook.export.org")
      (with-current-buffer (find-file-noselect "thebook.export.org")
        (lens/generate-custom-ids)
        (save-buffer)
        (org-html-export-to-html))
      (kill-buffer (get-file-buffer "thebook.export.org"))
      (delete-file "thebook.export.org")

      (rename-file "thebook.export.html" "thebook.html" t))
...
For historical reasons, and compatibility with other languages and tooling, Glint strings have a NUL byte (value of zero) appended.

*String literals are always a fixed array of =byte=*, with the length set long enough to store the contents of the string, /but not the NUL byte/. The NUL byte is guaranteed to be there, but, /it is not encoded in the type/. This means you do not have to add NUL checks to every loop; they are effectively not there, from the Glint side of things.
...
# TODO: When does implicit conversion occur?
...
To convert a value of some type to some other type, you can explicitly cast a value.

To do this, simply invoke the type with a single argument of the type you'd like to cast.

Note that, in doing the conversion, the value may be altered in some way.
...
To explicitly perform default initialization, provide the empty compound literal as the cast expression. This works with any (default-initializable) type.
#+begin_src glint
  byte !{}; ;; results in `\0`
#+end_src

The above expression results in =`\0`= because =byte= gets zero-initialized by default.
...
- =enum= arguments get converted to a fixed =byte= array representing their name.
- =type= arguments get stringified, embedded into the program, and printed.
- Otherwise, a call to =format=, a function that returns =[byte]= is inserted, the contents of that returned dynamic byte array are printed, and then the dynamic array is freed.
...
An array view is a non-owning view of a contiguous collection of elements of a given type. This is like spans in the C++ language.
...
# Fixed array interaction with ranged for loop expression
A fixed array is compatible with a ranged for loop expression (more on that later).
#+begin_src glint
  for i in "foo",
    print i, `\n`;
#+end_src
...
#+begin_src glint
  ...
  ;; Address Calculation
  foo/first :: foo[0]; ;; byte.ptr
  foo/second :: foo[1]; ;; byte.ptr

  ;; Element Access
  foo/first-value :: @foo[0];

  ;; Modification (here be dragons)
  @bar[0] := `b`;
#+end_src
...
# TODO: Array View Initialization
...
=switch= selects a control flow depending on which member of an enum type is held.

=switch= must handle every possible member an enum type may have.

#+begin_src glint
  switch value_of_enum_type, {
    .enumerator0 expr;
    .enumerator1 expr;
  };
#+end_src
...
To avoid long chains of =if= expressions comparing against each enumerator of a given enum type, the =switch= expression may be used in turn. In fact, there are more benefits to using =switch=, such as the compile-time guarantee that every possible enumerator a valid value may hold has a control flow to handle it.
...
The switch expression will execute exactly one of it's named body expressions: the one matching the name of the value currently stored in the instance of the given enum type.

The program would not compile if there were no body expression named =x= or if there were no body expression named =y=; every declared enumerator within the enum type must have an expression associated with it in a =switch= expression on that enum type.
...
# TODO: example of an actually useful template.
...
A templated type is a template expression that defines a type.
...
A templated function is /like/ a template expression that defines a function declaration.
...
An expression is an expression, er, at least according to the law of identity. Most times, we want the /result/ of an expression (the expression acquired /after/ evaluating the given expression). This means variable references like =foo= get resolved to their stored value---usually exactly what you want. However, sometimes, we want to operate on the expression itself /before/ evaluation. An expression expression is an unevaluated form of expression.
...
#+begin_src glint
  ...
  typeof foo; ;; expr (type)

  foo.type; ;; int (type)
  foo.name; ;; void---use with declarations
  foo.source; ;; "69" ([byte 3])

  ;; Expression Multiplication
  3 foo; ;; !{ 69; 69; 69; } ([expr 3])
#+end_src
...
#+begin_src glint
  ...
  foldl foos, binary+, 0; ;; 69
  foldl foos, binary+, 351; ;; 420
...
#+begin_src glint
  ;; program <name> allows naming a non-library Glint module, allowing self-
  ;; referencing metaprogramming.
  program P;
  ;; module P;

  P.name; ;; "P" ([byte 2])
  P.expressions; ;; list of all top-level expressions
  P.declarations; ;; list of all top-level declaration expressions
  P.imports; ;; list of module expressions
#+end_src
...
#+begin_src glint
  ...
  foldl my/numbers.expressions, binary+, 0;
#+end_src

The result of running the above program would be =69=.
...
When you build a module =Foo=, the =Foo.gmeta= file will be generated (alongside any output you've asked LCC to generate from the IR that the Glint frontend generates). When building a program that imports Glint module =Foo=, the compiler searches it's include directories (specified via =-I= on the command line) for the =Foo.gmeta= metadata file. If this file cannot be found in any of the include directories, or if it is of an unexpected file format, the Glint compiler issues an error and stops compilation, as it cannot continue to build a program that imports a module it knows nothing about.
...
#+begin_src glint
  value : int
    69;
  ;; Unary & Operator Yields A Pointer
  pointer : int.ptr
    &value;
  ;; Unary @ Operator Requires A Pointer, Returns Pointer's Element Type
  loaded_value : int
    @pointer;
#+end_src
...
A type may be accessed to query information about it. This is similar to accessing members of a struct, but /definitely not the same/.

All types support the following accessors.
#+begin_src glint
  int.bits; ;; 64
  int.bytes; ;; 8
  int.align; ;; 8
  int.ptr; ;; pointer to integer type
  int.ref; ;; reference to integer type
#+end_src

All types that contain a singular relevant type support the =elem= accessor.
#+begin_src glint
  ;; Enums
  foo/enum :: enum {
    x :: 0;
  ...
  foo/array : [byte 8];
  foo/array.elem; ;; byte
  ;; Same for dynamic arrays and array views.
...
Given that there isn't a special syntax for type introspection, it is likely a good idea to avoid naming enumerators the same exact thing as any valid type accessor.

*** On bits vs bytes

Bit alignment is not something that is supported by any real machine or language. By definition, a byte is the minimum addressable unit in any computer. So, when it comes to alignment of an address, bytes are relevant. However, when it comes to the size of a type in Glint, bits are definitely relevant, since you may have a =u27= or an =s42=.
